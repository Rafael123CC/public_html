...
$table->setTitle("Lista de TVs");
//SELECT `id`, `local`, `resX`, `resY` FROM `listaTVs` WHERE 1
//select the table in the datebase
$table->prepareTable("listaTVs");

//list of fields for list, new, edit and import records
$table->setFieldsAtive("id, local, resX, resY, layout",'list');
$table->setFieldsAtive("local, resX, resY, layout", 'new');
$table->setFieldsAtive("local, resX, resY, layout", 'edit');
$table->setFieldsAtive("local, resX, resY, layout", 'csv');
...
$table->setFieldList("layout",1," SELECT `id`,`nome` FROM `layouts` ORDER BY `id`");
